...
<body class="no-sidebar is-preload">
	...
	<div id="header">
		...
		<div class="inner">
			<header>
				<h1>
					<a href= "../attraction/Attraction_Map" id="logo">중 문</a>
				</h1>
			</header>
		</div>
						...
						<section>
							...
							<div class="row gtr-50" style="align-items: center">
								<c:forEach var="admin" items="${fa_list }">
									<c:choose>
										<c:when test="${admin.lnum == 12 && admin.fanum == 1}">
											<div class="col-4">
								...
								<div class="page_wrap">
									<div class="page_nation">
											...
											<c:if test="${adpageMaker.prev }">
												<a class="arrow prev"
													href="Jungmun_Attraction?pageNum=${adpageMaker.adStartPage -1 }">Previous</a>
											</c:if>
											<!-- 2. 페이지번호 활성화 여부 -->
											<c:forEach var="num" begin="${adpageMaker.adStartPage }"
												end="${adpageMaker.adEndPage}">
												<a class="${adpageMaker.adcri.pageNum == num ? 'active':'' }" href="Jungmun_Attraction?num1=12&count_oracle=${adpageMaker.adcri.count_oracle}&adpageStart=${adpageMaker.adcri.adpageStart}&pageNum=${num }">${num }</a>
											</c:forEach>
											<!-- 3. 다음 버튼 활성화 여부 -->
											<c:if test="${adpageMaker.next }">
												<a class="arrow next"
													href="Jungmun_Attraction?pageNum=${adpageMaker.adEndPage +1 }">Next</a>
											</c:if>
