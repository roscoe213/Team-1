							...
							<div class="page_wrap">
									<div class="page_nation">
											...
											<c:if test="${adpageMaker.prev }">
												<a class="page-link"
													href="Aewol_Food?pageNum=${adpageMaker.adStartPage -1 }">Previous</a>
											</c:if>
											<!-- 2. 페이지번호 활성화 여부 -->
											<c:forEach var="num" begin="${adpageMaker.adStartPage }"
												end="${adpageMaker.adEndPage}">
												<a class="${adpageMaker.adcri.pageNum == num ? 'active':'' }" href="Aewol_Food?num1=1&count_oracle=${adpageMaker.adcri.count_oracle}&adpageStart=${adpageMaker.adcri.adpageStart}&pageNum=${num }">${num }</a>
											</c:forEach>
											<!-- 3. 다음 버튼 활성화 여부 -->
											<c:if test="${adpageMaker.next }">
												<a class="arrow next"
													href="Aewol_Food?pageNum=${adpageMaker.adEndPage +1 }">Next</a>
											</c:if>
